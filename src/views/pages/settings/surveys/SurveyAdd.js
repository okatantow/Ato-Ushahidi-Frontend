...
function SurveyAdd(props) {
    ...
    const addRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('createDeploymentSurvey',
            JSON.stringify(data),
            {
                headers: {
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Survey Added Successfully" } }))
    ...
    const updateRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('updateDeploymentSurvey',
            JSON.stringify(data),
            {
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Survey Updated Successfully" } }))
    ...
    const deleteRecord = async (idD) => {
        setPending(true);
        const results = await axiosInstance.post('deleteDeploymentSurvey',
                ...
                <Card.Header>
                    <Card.Title as="h4">
                        ...
                        <div className="flex items-start justify-between">
                            <span>Surveys | <span className="text-[0.6em] capitalize"> {props?.formType} </span> </span>
                ...
                <Card.Body >
                    ...
                    >
                        ...
                        <div className="md:min-h-[450px] relative">
                            ...
                            <div>
                                <div className="mb-6">
                                    <label for="survey_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Survey Name</label>
                                    <input type="text" onChange={handleChange} name="survey_name" value={formValue.survey_name} id="survey_name" className=" focus:bg-white bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Name of Survey" required />
                                </div>

                                <div className="mb-6">
                                    <label for="survey_description" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Describe the survey</label>
                                    <textarea id="survey_description" onChange={handleChange} name="survey_description" value={formValue.survey_description} rows="3" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300  dark:bg-gray-700 dark:border-gray-300 dark:placeholder-gray-400 dark:text-white  focus:bg-white" placeholder="Survey Description..."></textarea>
